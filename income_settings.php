...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_POST['action'] == 'save') {
    ...
    if ($item_code > 0 && !empty($item_name)) {
        ...
        $sql = "INSERT INTO account_items (church_id, target_year, item_code, item_name) 
                VALUES ('$church_id', $target_year, $item_code, '$item_name')
...
// 2. 리스트 불러오기 (코드순 정렬)
// 데이터 불러오기
$result = $conn->query("SELECT * FROM account_items WHERE target_year = $target_year AND church_id = '$church_id' AND item_type = 'INCOME' ORDER BY item_code ASC");
?>
<!DOCTYPE html>
<html lang="ko">
<head>
    ...
    <style>
        ...
        .summary-card { border-left: 4px solid var(--gold); background: #1a1a14; }
        
        .item-code { font-family: 'monospace'; font-weight: bold; color: var(--gold); width: 60px; font-size: 15px; }
        .item-info { flex: 1; font-size: 15px; cursor: pointer; }
        
        /* 삭제 버튼 */
        .btn-delete { 
            color: #555; padding: 10px; font-size: 16px; transition: 0.2s;
        }
...
<div class="container">
		<div class="header">
			<h1><i class="fa-solid fa-coins"></i> <?= $target_year ?> 수입 항목 설정</h1>
		</div>

		<div class="input-section">
			<form action="income_settings.php" method="POST">
				<input type="hidden" name="action" value="save">
				<div class="input-group">
					<input type="number" name="item_code" id="item_code" class="input-code" placeholder="코드" required>
					<input type="text" name="item_name" id="item_name" class="input-name" placeholder="항목명 입력" required>
			...
			<?php if($result->num_rows > 0): ?>
				...
				<div class="item-card <?= $is_summary ? 'summary-card' : '' ?>">
					...
					<div class="item-info" onclick="editItem('<?= $row['item_code'] ?>', '<?= $row['item_name'] ?>')">
						...
						<?php if($is_summary): ?><span style="font-size:10px; color:var(--gold); margin-left:5px;">[소계]</span><?php endif; ?>
					</div>
				...
				<?php endwhile; ?>
			<?php else: ?>
				<div style="text-align:center; padding:50px; color:#444;">등록된 항목이 없습니다.</div>
			<?php endif; ?>
		</div>
</div>
...
<div class="bottom-nav">
    <a href="income_settings.php" class="nav-item active"><i class="fa-solid fa-hand-holding-dollar"></i><br>수입설정</a>
    <a href="expense_settings.php" class="nav-item"><i class="fa-solid fa-money-bill-transfer"></i><br>지출설정</a>
    <a href="income_main.php" class="nav-item "> <i class="fa-solid fa-file-lines"></i><br>돌아가기</a>
</div>

<script>
// 1. 수정 기능: 항목 터치 시 입력창으로 이동
...
function confirmDelete(code, name) {
    if(confirm(`[${code}] ${name}\n이 항목을 정말 삭제하시겠습니까?`)) {
        location.href = `income_settings.php?action=delete&code=${code}`;
    }
}
</script>
